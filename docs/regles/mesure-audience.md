...
| « Soumis » retenu | TCF rapide : `submitted_at`. Civique : `submitted_at` **et** `submitted_answered_count ≥ civicSubmittedMinAnsweredRatio × submitted_question_count` (0,8, contrôle C, V076) ; mesure absente = inconnu, non compté. Appliqué une fois (`SuiviReadRepository.SOUMIS_RETENU`, dans `RUNS`) : tunnel, ratios, KPI, activité, « jamais rattachées » |
