...
/**
 * Entrée de l'examen blanc EE (3 tâches enchaînées, évaluation IA + CECRL
 * plancher). Réservé aux abonnés Intégral : une session = 3 soumissions, au-delà
 * du quota gratuit (2 essais), donc paywall à l'entrée pour les non-abonnés.
 */
export default function EeExamsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    productionApi
      .listMine({epreuve: "TCF_EE", limit: 100})
      .then((list) => {
        if (cancelled) return;
  ...
  async function start() {
    ...
    try {
      ...
      router.push(`/entrainement/tcf/ee/session/${attempt.id}`);
    } catch (e) {
  ...
  const passedCount = useMemo(() => past.length, [past]);

  if (status === "loading") return <div className={ds.gate} />;
  if (!user) return <ModuleDetailGate next="/entrainement/tcf/ee/examens" />;
      ...
      <main className={hub.hub}>
        ...
        <div className={prod.card}>
          ...
          <p className={prod.consigne} style={{fontSize: 14}}>
            Vous rédigez les 3 productions écrites (message, récit, point de vue argumenté). À la
            fin, l&apos;IA évalue chaque tâche et vous attribue un niveau CECRL global (le plancher
            des 3 tâches).
          </p>
          <div className={prod.metaRow}>
            ...
            <span className={prod.metaChip}>
              <Clock size={13} strokeWidth={2} />~60 min
            </span>
            <span className={prod.metaChip}>
          ...
          <div className={prod.actions} style={{justifyContent: "flex-start", marginTop: 16}}>
            ...
            >
              ...
              {starting ? "Préparation…" : isPremium ? "Commencer l'examen blanc" : "Débloquer l'examen blanc"}
            </button>
          </div>
          {!isPremium && (
            <p className={prod.draftNote} style={{marginTop: 10}}>
              L&apos;examen blanc complet est réservé aux abonnés Intégral. Vous pouvez tester
              gratuitement l&apos;expression écrite via « S&apos;entraîner par tâche » (2 essais
              offerts).
            </p>
          )}
        </div>

        {!loading && passedCount > 0 && (
          <>
            ...
            <div className={hub.list}>
              {past.map((s) => (
                <button
                  ...
                  type="button"
                  className={prod.row}
                  onClick={() => router.push(`/entrainement/tcf/ee/session/${s.attemptId}`)}
                >
                  <span className={prod.rowChip}>
                    <FileStack size={14} strokeWidth={2.2} />
                  </span>
                  <span className={prod.rowBody}>
                    <span className={prod.rowTitle}>Examen blanc EE</span>
        ...
        <PaywallSheet
          ...
          message="L'examen blanc d'expression écrite (3 tâches + évaluation IA) est réservé aux abonnés Intégral, qui débloque aussi tout le TCF, le civique et les examens blancs illimités."
        />
      </main>
    </DualChromeShell>
  );
