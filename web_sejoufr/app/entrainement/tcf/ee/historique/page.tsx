...
/**
 * Historique des productions EE : sessions d'examen blanc (≥ 2 tâches partageant
 * un attempt) et entraînements libres (1 tâche). Parité `ProductionHistoryScreen`.
 */
export default function EeHistoryPage() {
  ...
  if (!user) return <ModuleDetailGate next="/entrainement/tcf/ee/historique" />;
    ...
    <DualChromeShell>
      <main className={hub.hub}>
        <HubDetailHeader
          ...
          title="Historique"
          subtitle="Expression écrite · examens et entraînements"
        />

        {loading ? (
          <p className={prod.loading}>Chargement…</p>
        ) : empty ? (
          <p className={prod.empty}>Aucune production EE pour l&apos;instant.</p>
        ) : (
          <>
            {sessions.length > 0 && (
              <>
                ...
                <div className={hub.list}>
                  {sessions.map((s) => (
                    <button
                      ...
                      className={prod.row}
                      onClick={() =>
                        router.push(`/entrainement/tcf/ee/session/${s.attemptId}`)
                      }
                    >
                      ...
                      <span className={prod.rowBody}>
                        ...
                        <span className={prod.rowSub}>
                          {s.items.length} tâche{s.items.length > 1 ? "s" : ""} ·{" "}
                          {formatDay(s.date)}
                        </span>
                      </span>
