 ...
 * "Démarrer", on POST /api/attempts (MOCK_EXAM) et on redirige vers la route
 * générique /sessions/<attemptId> où le QuestionRunner prend le relais.
 */
export function ExamBriefingClient({ exam }: { exam: ExamTemplateSummary }) {
  ...
  // Cet examen est accessible si : free (1 par module offert), ou abonné pour
  // ce module. Sinon : paywall sheet.
  const accessGranted = exam.free || isPremiumForModule;
  ...
  if (!user) {
    return (
      <main className="brf-gate">
        <p>Connectez-vous pour passer un examen blanc.</p>
        <Link href={`/connexion?next=/examens-blancs/${exam.slug}`} className="btn btn-blue">
          Se connecter
        </Link>
      </main>
    );
            ...
            {!accessGranted ? (
              ...
              <div className="brf-cta-row">
                <button
                  ...
                  disabled={starting}
                >
                  {starting ? "Préparation…" : "Démarrer l'examen →"}
                </button>
