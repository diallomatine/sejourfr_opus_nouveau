...
/**
 * Client « avant / apres » du diagnostic, variante Anthropic ({@code tool_use}).
 *
 ...
 * candidat. Sans lui, un changement de fournisseur ferait disparaitre le bloc de
 * conversion de l'ecran de resultat sans qu'aucune alerte ne le signale.
 *
 * <p>Aucun rejeu : cet appel est best-effort (cf.
 * {@link DiagnosticExempleCibleLlmClient}).
 */
public class DiagnosticExempleCibleAnthropicClient implements DiagnosticExempleCibleLlmClient {

    private static final Logger log =
        LoggerFactory.getLogger(DiagnosticExempleCibleAnthropicClient.class);
    ...
    public DiagnosticExempleCibleAnthropicClient(
    ...
    public Outcome produire(String systemPrompt, String userPrompt) {
        if (!connection.isConfigured()) {
            throw new AiEvaluationException(
                "Anthropic : cle API non configuree — bloc « avant / apres » ignore.");
        }
        ...
        try {
            ...
            throw new AiEvaluationException(
                "Anthropic exemple cible indisponible : " + e.getMessage(), e);
        }
        if (response == null) {
            throw new AiEvaluationException("Reponse Anthropic exemple cible vide");
        }

        Outcome outcome = parseToolUseOutcome(response);
        log.info("Anthropic diagnostic exemple cible OK input={} output={} duration={}ms",
    ...
    Map<String, Object> buildRequestBody(String systemPrompt, String userPrompt) {
        ...
        tool.put("name", DiagnosticExempleCibleFields.TOOL_NAME);
        tool.put("description", DiagnosticExempleCibleFields.TOOL_DESCRIPTION);
        ...
        body.put("tool_choice",
            Map.of("type", "tool", "name", DiagnosticExempleCibleFields.TOOL_NAME));
        return body;
    }
    ...
    private Outcome parseToolUseOutcome(JsonNode response) {
        ...
        if (!content.isArray() || content.isEmpty()) {
            throw new AiEvaluationException("Reponse Anthropic exemple cible sans bloc content");
        }
        ...
        for (JsonNode block : content) {
            ...
            if ("tool_use".equals(type) && DiagnosticExempleCibleFields.TOOL_NAME.equals(name)) {
                toolBlock = block;
                break;
            }
        }
        if (toolBlock == null) {
            throw new AiEvaluationException(
                "Pas de bloc tool_use " + DiagnosticExempleCibleFields.TOOL_NAME
                    + " dans la reponse");
        }
        JsonNode input = toolBlock.path("input");
        if (input.isMissingNode() || input.isNull()) {
            throw new AiEvaluationException("Bloc tool_use sans champ input (exemple cible)");
        }
        ...
        try {
            ...
            throw new AiEvaluationException(
                "Input du tool_use non deserialisable (exemple cible) : " + e.getMessage(), e);
        }
